...
\todo{Notes: lazy loading, advantage of array type data objects, code example of loading data and accessing content + custom annotations, rawio features + lazy loading), saving content to open formats (Nix!)} 

\begin{figure}
    \centering
    \todo{include IO figure here}
%     \includesvg[width=\textwidth,pretex=\relscale{0.8}]{./figures/neo07_schema.svg}
    \caption{\software{Neo} embedding. \software{Neo} 0.7. supports a number of file formats (bottom) and provides an interface for many advanced tools for simulation, visualization and analysis.}
    \label{fig:neo_ios}
\end{figure}
